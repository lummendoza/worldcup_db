...
echo  "$($PSQL "(Select name FROM games INNER JOIN teams ON games.winner_id = teams.team_id WHERE year=2014 and ROUND ='Eighth-Final') UNION (SELECT name FROM games FULL JOIN teams ON games.opponent_id = teams.team_id WHERE year=2014 and ROUND ='Eighth-Final') ORDER BY name")"
